...
Dasselbe Schema, aber ohne Umpolung — die sieben `pa`-Items sind alle gleich gepolt:

```{r scale-pop}
allbus <- allbus |>
  mutate(
    populismus = row_means(pick(pa29, pa30, pa31, pa32, pa33, pa34, pa35),
                           min_valid = 6)
  )
...
2. Umpolen der inversen Items `mm02` und `mm05` zu `mm02r` und `mm05r` mit dem `"rev"`-Shortcut.
3. Skala **Islamfeindlichkeit** als Mittelwert der sechs (jetzt gleichpoligen) Items — mindestens 5 von 6 müssen valide sein.
4. Skala **Populismus** als Mittelwert der sieben `pa`-Items — mindestens 6 von 7 müssen valide sein.
