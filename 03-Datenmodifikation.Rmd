...
**Anmerkung:** Der `%>%` Operator gehört zu den `infix operators`.
...
**Beispielcode:**
```{r, echo=T, collapse=T, comment="#>"}
# Nur Einstellung zu Leistungsprinzip (im19) im neuen Datensatz ds1 speichern
ds1 <- allbus2018 %>% 
  select(im19)
...
```{r, echo=T, collapse=T, comment="#>"}
# Befürwortung von Ungleichheit (im20) aus ds1 löschen
ds2 <- ds1 %>% 
  select(-im20)
...
**Anmerkung:** Wir benutzen die Packete *sjmisc* und *sjlabelled* zur Konvertierung von Datentypen, weil die Anwendung der "klassischen" Funktionen alle Beschriftungen unseres Datensatzes entfernen würde.
...
# Geschlecht in Faktor mit numerischen levels konvertieren (Äquivalent zu factor())
# Sinnvoll bei Variablen mit mehr als zwei Ausprägungsmerkmalen
...
# Sinvoll bei binären Ausprägungsmerkmalen
...
Die Verwendung der Funktion `rev_lvl()` aus *sjmisc* hat den großen Vorteil, dass unser Faktor praktisch rekodiert wird und damit alle Beschriftungen richtig zugewiesen werden. 
...
Zuletzt wollen wir lernen, wie wir Beschriftungen abrufen und verändern können. 
...
Für dieses Kapitel verzichten wir auf eine Darstellung der `Syntax`, da die Funktionen individuell angepasst werden müssen.
...
# Abrufen der Variablenbeschriftung 
get_label(ds$im19)

# Abrufen der Beschriftungen von Ausprägungsmerkmalen
get_labels(ds$im19)

# Löschen aller Beschriftungen
ds <- ds %>% 
  remove_all_labels()
