...
Mit dem Befehl `rename()` haben wir in unserem Datensatz `datensatz` die Variable `variable1` in `variableA` umbenannt. Der neue Name muss folgend immer auf der *linken* Seite der Gleichung stehen.
...
Mit dem Befehl `split_var()` können nummerische Variablen in gleich große Gruppen (Befragtenanzahl) aufgeteilt werden. Die Anzahl der Gruppen wird dabei mit dem Argument `n=` festgelegt.
...
Entgehgen dem vorherigen Befehl `split_var()`, ist hier der Fokus die Gruppen nach dem Wertebereich einzuteilen. Mit dem `group_var()` Befehl wird eine nummerische Variable in einer neuen Variable zu Gruppen mit gleichgroßem Wertebereich gruppiert. Mit dem Argument `size=` wird der Wertebreich der einzelnen Gruppen festgelegt.
...
Mit dem `row_count()` Befehl können bestimmte festgelegte Ausprägungen zielenweise gezählt und die Anzahl dieser Ausprägungen in einer neue Variable abgelegt werden. Dieser Befehl ist daher mit dem `count` Befehl in `SPSS` vergleichbar.
...
In der Syntax wird für den `datensatz` über alle Variable hinweg mit dem Argument `count = 1` alle Ausprägungen mit dem Wert 1 gezählt. (In dieser From wird keine neue Variable mit der Anzahl gebildet, sondern das Ergebnis direkt in der Console ausgegeben. In der Form des folgenden Beispielcodes wird die Anzahl der gezählten Ausprägungen unter dem Variablenname `rowcount` im Datensatz `ds` abgelegt.)
...
Zur Bildung eines Summenindexes kann der Befehl `row_sums()` verwendet werden. Dafür wird zeilenweise die Summe der Werte über alle Variablen eines Datensatzes gebildet.
...
**Wichtig!** Das Argument `n =` bestimmt die Anzahl der gültigen Werte, welche zur Bildung der Summe vorliegen müssen. Da es keinen Default für dieses Argument gibt, muss `n` definiert werden. Mit `n = Inf`, wie im Bespiel, wird definiert, dass für alle Variablen ein gültiger Wert vorliegen muss.

In der Syntax wird für den `datensatz` über alle Variable zeilenweise die Summe der einzelnen Werte gebildet. (In dieser From wird keine neue Variable mit der Summe gebildet, sondern das Ergebnis direkt in der Console ausgegeben. In der Form des folgenden Beispielcodes wird die Summe der Werte unter dem Variablenname `rowsums` im Datensatz `ds` abgelegt.)
...
Analog zum vorherigen Befehl, kann mit `row_means` ein Mittelwertindex gebildet werden. Dafür wird zeilenweise die Summe der Werte über alle Variablen eines Datensatzes gerechnet und durch die Anzahl der Variablen geteilt.
...
**Wichtig!** Wie auch bei dem vorherigen Befehl muss das Argument `n =` definiert werden.

In der Syntax wird für den `datensatz` über alle Variable zeilenweise die Summe der einzelnen Werte und durch die Anzahl der Variablen geteilt. (In dieser From wird keine neue Variable mit der Summe gebildet, sondern das Ergebnis direkt in der Console ausgegeben. In der Form des folgenden Beispielcodes wird die Summe der Werte unter dem Variablenname `rowmeans` im Datensatz `ds` abgelegt.)
...
Mit der Funktion `set_na()` haben wir in `variable1` die Ausprägungsmerkmale `1` und `3` als `NA` definiert. 
...
Zum Ausschluss von Fällen (rows) aus dem Datensatz kann der Befehl `drop_na()` verwendet werden.
...
Wie in der Unterscheidung der Syntax deutlich wird, können Fälle dabei aufgrund fehlender Werte bei einzelnen Variable aussortiert werden oder aufgrund eines einzelnen fehlenden Wertes im ganzen Datensatz. Bspw. kann mit dem zweiten Ansatz (`#Ganzer Datensatz`) der ganze `datensatz` um Fälle mit fehlenden Werten bereinigt werden, sodass ausschließlich vollständige Fälle im `datensatz` vorliegen.
