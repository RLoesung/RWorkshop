...
plot_likert(allbus2018[c("im19", "im20", "im21", "id01")], grid.range = c(.8, .8), wrap.legend.labels = 11,
            wrap.labels = 12, values = "sum.inside", show.prc.sign = T, weight.by = allbus2018$wghtpew)
```
...
```{r echo=T, message=FALSE, warning=FALSE, collapse=T, comment="#>"}
# Verteilung Alter nach Geschlecht und Statistiken

allbus2018 %$% 
  plot_grpfrq(age, sex, weight.by = wghtpew, show.values = F, type = "line", show.summary = T, summary.pos = 1)
```

```{r echo=T, message=FALSE, warning=FALSE, collapse=T, comment="#>"}
# Verteilung Leistungsprinzip nach Schichtzugehörigkeit (id02) 
...
```{r echo=T, message=FALSE, warning=FALSE, collapse=T, comment="#>"}
# Verteilung Leistungsprinzip (im19) nach Geschlecht und Statistiken

allbus2018 %$% 
  plot_grpfrq(im19, sex, weight.by = wghtpew, show.values = T, type = "bar", show.summary = T, summary.pos = "1", show.axis.values = T, show.prc = T, ylim = c(0, 850))
```

```{r echo=T, message=FALSE, warning=FALSE, collapse=T, comment="#>"}
# Verteilung Leistungsprinzip (im19) nach Geschlecht und Ost West
# und Statistiken

allbus2018 %$% 
  plot_grpfrq(im19, sex, intr.var = eastwest, weight.by = wghtpew, type = "violin", show.summary = T, summary.pos = "1")
```
...
```{r, echo=T, collapse=T, comment="#>"}
# Verteilung soziale Ungleichheit aus Makroperspektive (im21) nach Ost West
# und Statistiken

allbus2018 %$% 
  plot_xtab(im21, eastwest, weight.by = wghtpew, type = "line", show.total = F, show.n = F,
