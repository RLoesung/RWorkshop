...
::: {.callout-tip collapse="true"}
**Übung 4 — Operatoren und NA.** Gegeben sei der Vektor `werte <- c(3, 8, NA, 12, 5, NA, 7)`. Bestimme (a) den Mittelwert ohne `NA`s, (b) wie viele Elemente größer als 5 sind, (c) ob mindestens ein Element fehlt.

**Lösung:**
```{r ex4-kap3-loesung, eval=FALSE}
werte <- c(3, 8, NA, 12, 5, NA, 7)
mean(werte, na.rm = TRUE)            # (a)
sum(werte > 5, na.rm = TRUE)         # (b) TRUE zählt als 1
any(is.na(werte))                    # (c)
```
:::
