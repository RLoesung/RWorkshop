...
### VIF händisch berechnen — in drei Schritten

Wir rechnen den VIF *genau nach der Formel oben* selbst — und zwar zuerst für **einen** Prädiktor, in drei sichtbaren Schritten. Beispiel: VIF für `populismus`.

**Schritt 1 — Hilfsregression.** Wir machen `populismus` zur *abhängigen* Variable und lassen alle anderen Prädiktoren des Hauptmodells sie vorhersagen:

```{r vif-step1}
hilfsmodell <- lm(populismus ~ educ_kat + age + eastwest + sex,
                  data = allbus, weights = wghtpew)
```

**Schritt 2 — R² der Hilfsregression ablesen.** Wie gut lassen die anderen Variablen sich Populismus vorhersagen?

```{r vif-step2}
r2 <- summary(hilfsmodell)$r.squared
r2
```

Etwa 18 % der Varianz von Populismus lassen sich aus den anderen Prädiktoren erklären.

**Schritt 3 — VIF-Formel anwenden.**

```{r vif-step3}
vif_populismus <- 1 / (1 - r2)
vif_populismus
```

Resultat: VIF ≈ 1.22 — weit unter der Warngrenze 5. Populismus hat also kein Multikollinearitäts-Problem mit den anderen Prädiktoren.

**Für die anderen Prädiktoren** wiederholst du dieselben drei Schritte mit jeweils einer angepassten Hilfsformel — `age ~ populismus + educ_kat + eastwest + sex` für den VIF von Alter, `eastwest ~ populismus + educ_kat + age + sex` für Region, und so weiter. In Übung 2 am Kapitelende rechnest du das für einen weiteren Prädiktor durch.

::: {.callout-note}
**Hintergrund: Kategoriale Prädiktoren und VIF.** Bei einem Faktor wie `educ_kat` (drei Stufen → zwei Dummy-Variablen, siehe @sec-dummy) bekommt jede Dummy-Variable streng genommen einen eigenen VIF. Spezialisierte Funktionen wie `car::vif()` (aus dem Paket `car`) rechnen für Faktoren stattdessen den **generalisierten VIF** (GVIF) — eine einzelne Kennzahl pro Original-Variable, deren Spalte `GVIF^(1/(2·Df))` direkt mit den VIF-Faustregeln vergleichbar ist. Für Modelle wie unseres (alle VIFs deutlich unter 2.5) ist der Unterschied praktisch unbedeutend — bei Modellen nahe an der Warngrenze lohnt es sich, GVIF gezielt nachzulesen.
:::
...
Rechne den VIF für den Prädiktor **`age`** händisch nach dem 3-Schritt-Muster aus @sec-vif (Hilfsregression → R² → 1 / (1 − R²)). Liegt der VIF unter der Warngrenze 5? Und: wie sähe die Hilfsformel aus, um den VIF von `eastwest` zu rechnen — du musst sie nicht durchrechnen, nur hinschreiben?
:::

::: {.callout-tip title="Lösung" collapse="true"}
```{r ex2-kap9-loesung, eval=FALSE}
# Schritt 1: Hilfsregression — age durch alle anderen Praediktoren erklaeren
hilfs_age <- lm(age ~ populismus + educ_kat + eastwest + sex,
                data = allbus, weights = wghtpew)

# Schritt 2: R² ablesen
r2_age <- summary(hilfs_age)$r.squared

# Schritt 3: VIF-Formel
1 / (1 - r2_age)

# Hilfsformel fuer den VIF von eastwest (nicht ausgerechnet):
# eastwest ~ populismus + educ_kat + age + sex
```
:::
