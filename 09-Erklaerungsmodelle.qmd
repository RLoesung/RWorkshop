...
vif_df <- tibble(praediktor = colnames(X)) |>
  mutate(
    VIF       = map_dbl(praediktor, \(p) {
                  aux_r2 <- summary(lm(X[, p] ~ X[, setdiff(colnames(X), p)]))$r.squared
                  1 / (1 - aux_r2)
                }),
    Tolerance = 1 / VIF
  )
...
tibble(praediktor = colnames(X)) |>
  mutate(VIF = map_dbl(praediktor, \(p) {
    1 / (1 - summary(lm(X[, p] ~ X[, setdiff(colnames(X), p)]))$r.squared)
  }))
```
:::
