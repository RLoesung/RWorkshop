...
::: {.callout-important}
**mariposa-Tipp: Direkt-Dispatch seit 0.6.3.1.** Das Ergebnis von `linear_regression()` *ist* ein `lm`-Objekt (es erbt direkt davon: `class = c("linear_regression", "lm")`). Alle Standard-R-Funktionen und `broom`-Generics dispatchen unmittelbar — du musst nichts mehr extrahieren:

- **base R**: `anova()`, `predict()`, `coef()`, `confint()`, `vcov()`, `residuals()`, `fitted()`, `plot()`
- **broom**: `tidy(modell, conf.int = TRUE)` (term-Tabelle mit KI), `glance(modell)` (Modell-Kennzahlen: R², F, AIC, BIC …), `augment(modell)` (Daten + Residuen + Leverage)

Wenn du *zusätzlich* die SPSS-Style-Tabellen aus mariposa brauchst, findest du sie als Slots am selben Objekt: `modell$coef_table`, `modell$anova_table`, `modell$model_summary` (mit `R_squared`, `adj_R_squared`, `std_error`).
:::
